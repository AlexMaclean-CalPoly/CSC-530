...
\begin{solution}
    Instead of representing vertex coloring as $k$ literals, each literal coresponding to one possible color, represent colors by each literal represented by a bit in a binary string. This encodes the colors as binary, using $\log k$ the number of variables.

    Edge constraint:
    $$ \bigwedge_{\langle v, w \rangle \in E} (\bigvee_{b \in B} p^b_v \land \neg p^b_w )$$
\end{solution}
...
\begin{solution}
    Start with the constraints generated by $P(G, C_{|V|})$, meaning each vertex has a unique color. We know that this is staisfiable. Next, add the constraint that none of verticies may be assigned the greatest color. Iteritevly add this constraint until the problem is no longer satisfiable. Since we are only adding constraint, the SAT solver can use the previously calculated work to incrementalally solve this problem. The coloring and the chromatic number correspond to the final satisfiable expression.
\end{solution}
...
\end{solution}
    ...
    \begin{solution}
	\begin{align*}
        \text{minimize}  	 \quad& \sum 2^c \cdot p^c_v \\
